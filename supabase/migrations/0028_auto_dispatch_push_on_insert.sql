...
--
-- NOTE: replace REPLACE_WITH_CRON_SECRET below with the CRON_SECRET value
-- configured in the project's Edge Function secrets (Settings → Edge
-- Functions → Secrets). The send-push path inserts rows with push_sent=true,
-- so this never double-sends them.
